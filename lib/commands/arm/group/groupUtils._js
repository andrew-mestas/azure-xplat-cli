...
function createDeploymentParameters(cli, subscription, resourceGroup, options, _) {
  ...
  if (options.parametersFile) {
    ...
    var missingObj = missingParameters(deploymentParameters, options, subscription, _);
...
function parsedContent(content) {
  ...
  try {
    return jsonlint.parse(JSON.minify(content));       
  } catch(e){
    try {
...
function estimatedErrorLine(content, e){
  ...
  var minified = JSON.minify(content);
...
function missingParameters(userParameters, options, subscription, _) {
  ...
  var deploymentParameters = __.keys(userParameters);
  ...
  var defaultParameters = __.keys(__.omit(templateContent.parameters, nonDefaultParameters));
  var filteredUserParameters = __.difference(deploymentParameters, defaultParameters); 
  if(nonDefaultParameters.length !== filteredUserParameters.length){
    var smallerList = nonDefaultParameters.length < filteredUserParameters.length ? 'Template' : 'Deployment';
    var missing = getMissingParameterDefintions(nonDefaultParameters, filteredUserParameters); 
    throw ' Template and Deployment "parameters" objects lengths do not match \n\t' + smallerList + ' Parameter file does not have { ' + missing + ' } defined.';
  }
  var missingParameters = nonDefaultParameters.filter(function(parameter) {
      try{
  ...
  missingParameters.forEach(function(missing) {
...
function getMissingParameterDefintions(templateParameters, deploymentParameters) {
  return __.difference(templateParameters, deploymentParameters).length === 0  ? __.difference(deploymentParameters, templateParameters) : __.difference(templateParameters, deploymentParameters);
}
